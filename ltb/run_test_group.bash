#!/usr/bin/env bash
# Run a single test group and open its waveform in gtkwave.
#
# Usage:
#   ./run_test_group.bash <test_group>
#
...
#   edge         — 1:x0_immutable
#
# To run a single numbered test use: ./run_test.bash <test_group> <test_#>

set -e

if [ -z "$1" ]; then
    echo "Usage: ./run_test_group.bash <test_group>"
    echo ""
...
# Map test name to task calls
case $TEST in
    alu_i)      CALLS="test_addi(); test_sign_extension();" ;;
    alu_r)      CALLS="test_add(); test_sub(); test_and(); test_or(); test_xor();" ;;
    mem)        CALLS="test_sw_lw();" ;;
    forwarding) CALLS="test_fwd_ex_ex(); test_fwd_mem_ex(); test_fwd_both_operands(); test_load_use_stall();" ;;
    sequences)  CALLS="test_running_sum(); test_store_load_chain(); test_alu_after_load();" ;;
    edge)       CALLS="test_x0_immutable();" ;;
    *)
        echo "Unknown test: '$TEST'"
        echo "Run ./run_test.bash with no arguments to see available tests."
        exit 1
        ;;
esac
...
# Generate a single-test wrapper on the fly
